...
import type { IMenuItem } from "./BaklavaContextMenuTypes";
...
const flippedX = computed(
  () => props.flippable && (rootIsFlipped.value.x || props.isFlipped.x),
);
const flippedY = computed(
  () => props.flippable && (rootIsFlipped.value.y || props.isFlipped.y),
);
...
const itemsWithHoverProperty = computed(() =>
  props.items.map((i) => ({ ...i, hover: false })),
);
...
watch([() => props.y, () => props.items], () => {
  ...
  rootIsFlipped.value.y =
    !props.isNested && props.y + height.value > parentHeight - 20;
});
...
const onMouseEnter = (_event: MouseEvent, index: number) => {
...
const onMouseLeave = (_event: MouseEvent, index: number) => {
  if (props.items[index]?.submenu) {
